...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:8085/api/users/register', formData);
    ...
    <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', minHeight: '100vh', width: '100%' }}>
      <form onSubmit={handleSubmit} style={{ width: '100%', maxWidth: '350px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          ...
          />
          <input 
            type="text" placeholder="Nombre de usuario (3-20 caracteres)" 
          ...
          />
          <input 
            type="text" placeholder="Zona donde vives" 
            value={formData.zona}
